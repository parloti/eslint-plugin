...
Normalizes `vi.mock(...)` and `vi.doMock(...)` factories when TypeScript rejects the returned module object, replacing broad or failed casting attempts with the smallest stable nested casts needed for the mocked module shape.
